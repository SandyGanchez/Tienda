...
async function esperarServidor() {
  for (let intento = 0; intento < 40; intento += 1) {
    try { await fetch(`${baseUrl}/public/tienda`); return; } catch { await new Promise(resolve => setTimeout(resolve, 250)); }
  }
...
async function crearPedido(clienteToken, cantidad, uuid = crypto.randomUUID(), extra = {}) {
  const respuesta = await peticion('/cliente/pedidos', {
    method: 'POST', body: JSON.stringify({ uuidPedido: uuid, items: [{ idPro: producto.idPro, cantidad }], ...extra })
  }, clienteToken);
...
async function preparar() {
  db = await mysql.createConnection({ host: process.env.DB_HOST, port: Number(process.env.DB_PORT || 3306),
  ...
  const [configuraciones] = await db.query('SELECT idConfiguracion FROM configuracion_transferencia WHERE idSuc = ?', [idSuc]);
  if (!configuraciones.length) {
    const [result] = await db.query(`INSERT INTO configuracion_transferencia
      (idSuc,banco,titular,numeroCuenta,instrucciones,activo) VALUES (?,'Banco de prueba','Pruebas Fase 2','00000001','Configuración temporal de pruebas',1)`, [idSuc]);
  ...
  const [clienteTemporal] = await db.query(`INSERT INTO cliente
  ...
  producto = productos[0]; stockOriginal = Number(producto.existenciaPro);
  ...
  return { idCliente, idCliente2:idClienteTemporal, idAdmin:Number(administradores[0]?.idEmp || 0), idCajero:Number(cajeros[0]?.idEmp || 0) };
}

async function ejecutar() {
  const ids = await preparar();
  servidor = spawn(process.execPath, [path.join(__dirname, 'index.js')], { cwd:__dirname, stdio:['ignore','pipe','pipe'] });
  ...
  comprobar('Configuración cliente protegida disponible', r.status === 200 && !('idConfiguracion' in r.body.configuracion), `HTTP ${r.status}`);
  if (ids.idAdmin) {
  ...
  comprobar('Pedido válido y reserva', creado.status === 201 && await stock() === stockOriginal - 2, `HTTP ${creado.status}`);
  comprobar('Precio y total reales', Number(creado.body.total) === Number(producto.precioVentaPro) * 2, `total ${creado.body.total}`);
  const repetido = await crearPedido(cliente1, 2, uuidIdempotente);
  comprobar('Idempotencia mismo cliente', repetido.status === 200 && repetido.body.idPedido === creado.body.idPedido && await stock() === stockOriginal - 2);
  ...
  comprobar('Cancelar restaura stock', r.status === 200 && r.body.estado === 'CANCELADO' && await stock() === stockOriginal);
  r = await cancelar(creado.body.idPedido, cliente1);
  ...
  comprobar('Stock insuficiente conserva inventario', r.status === 409 && await stock() === stockOriginal, `HTTP ${r.status}`);
  ...
  const exitosos = [conA,conB].filter(x=>x.status===201), conflictos=[conA,conB].filter(x=>x.status===409);
  const stockConcurrente = await stock();
  comprobar('Concurrencia último producto', exitosos.length===1 && conflictos.length===1 && stockConcurrente===0,
  ...
  await db.query('UPDATE pedido_cliente SET fechaLimitePago=DATE_SUB(NOW(),INTERVAL 1 MINUTE) WHERE idPedido=?',[expira.body.idPedido]);
  ...
  const form = new FormData(); form.append('comprobante',new Blob([png],{type:'image/png'}),'prueba.png');
  r = await peticion(`/cliente/pedidos/${comprobante.body.idPedido}/comprobante`,{method:'POST',body:form},cliente1);
  ...
  const ajeno=await crearPedido(cliente1,1);const formAjeno=new FormData();formAjeno.append('comprobante',new Blob([png],{type:'image/png'}),'ajeno.png');
  r=await peticion(`/cliente/pedidos/${ajeno.body.idPedido}/comprobante`,{method:'POST',body:formAjeno},cliente2);
  comprobar('Subida a pedido ajeno rechazada',r.status===404);await cancelar(ajeno.body.idPedido,cliente1);

  const vencido=await crearPedido(cliente1,1);await db.query('UPDATE pedido_cliente SET fechaLimitePago=DATE_SUB(NOW(),INTERVAL 1 MINUTE) WHERE idPedido=?',[vencido.body.idPedido]);
  const formVencido=new FormData();formVencido.append('comprobante',new Blob([png],{type:'image/png'}),'vencido.png');
  r=await peticion(`/cliente/pedidos/${vencido.body.idPedido}/comprobante`,{method:'POST',body:formVencido},cliente1);
  comprobar('Comprobante de pedido expirado rechazado',r.status===409&&await stock()===stockReservado);

  const invalido=await crearPedido(cliente1,1);const formInvalido=new FormData();formInvalido.append('comprobante',new Blob(['no es png'],{type:'image/png'}),'falso.png');
  r=await peticion(`/cliente/pedidos/${invalido.body.idPedido}/comprobante`,{method:'POST',body:formInvalido},cliente1);
  comprobar('Firma MIME inválida rechazada',r.status===400);await cancelar(invalido.body.idPedido,cliente1);
  const grande=await crearPedido(cliente1,1);const formGrande=new FormData();formGrande.append('comprobante',new Blob([Buffer.alloc(5*1024*1024+1)],{type:'image/png'}),'grande.png');
  r=await peticion(`/cliente/pedidos/${grande.body.idPedido}/comprobante`,{method:'POST',body:formGrande},cliente1);
  comprobar('Comprobante mayor a 5 MB rechazado',r.status===400);await cancelar(grande.body.idPedido,cliente1);
...
async function limpiar() {
  ...
  try {
    ...
    for (const idPedido of pedidosPrueba) {
      const [pedidos] = await db.query('SELECT estado,comprobanteRuta FROM pedido_cliente WHERE idPedido=? FOR UPDATE',[idPedido]);
      if (!pedidos.length) continue;
      if (['PENDIENTE_PAGO','EN_REVISION'].includes(pedidos[0].estado)) {
        const [detalles]=await db.query('SELECT idPro,cantidad FROM detalle_pedido_cliente WHERE idPedido=? ORDER BY idPro',[idPedido]);
        for(const detalle of detalles) await db.query('UPDATE productos SET existenciaPro=existenciaPro+? WHERE idPro=?',[detalle.cantidad,detalle.idPro]);
      }
      if(pedidos[0].comprobanteRuta){const ruta=path.join(__dirname,'uploads','comprobantes',path.basename(pedidos[0].comprobanteRuta));if(fs.existsSync(ruta))fs.unlinkSync(ruta);}
    ...
    await db.commit();
  } catch(error){await db.rollback().catch(()=>undefined);console.error('TEST_CLEANUP_ERROR',error.message);}
  await db.end();
}

ejecutar().catch(error=>{console.error('FASE_2_BACKEND_FAIL',error.message);process.exitCode=1;}).finally(limpiar);
