...
async function crearPedido(clienteToken, cantidad = 2) {
  const respuesta = await peticion(
    '/cliente/pedidos',
    {
      method: 'POST',
      body: JSON.stringify({
        uuidPedido: crypto.randomUUID(),
        items: [{ idPro: producto.idPro, cantidad }],
      }),
    },
    clienteToken,
  );
...
async function subirComprobante(idPedido, clienteToken) {
  ...
  const respuesta = await peticion(
    `/cliente/pedidos/${idPedido}/comprobante`,
...
async function preparar() {
  ...
  const [admins] = await db.query(
    `SELECT e.idEmp FROM empleados e JOIN cargo c ON c.idCargo=e.idCargo
    WHERE e.estadoEmp=1 AND c.nombreCargo='ADMINISTRADOR' AND c.idSuc=? LIMIT 1`,
    [idSuc],
  );
  const [cajeros] = await db.query(
    `SELECT e.idEmp FROM empleados e JOIN cargo c ON c.idCargo=e.idCargo
    WHERE e.estadoEmp=1 AND c.nombreCargo='CAJERO' AND c.idSuc=? LIMIT 1`,
    [idSuc],
  );
  if (!admins.length || !cajeros.length)
    throw new Error('Se requiere un administrador y un cajero activos en la sucursal.');
  ...
  if (!configuraciones.length) {
    const [result] = await db.query(
      ...
      VALUES (?,'Banco snapshot F3','Titular F3','123456789012345678','F3-001','Datos temporales Fase 3',1)`,
      [idSuc],
    );
  ...
  const [cliente] = await db.query(
    ...
    VALUES ('Cliente','Fase Tres',?,?,1)`,
    [`fase3-${sufijo}@example.invalid`, `fase3-${sufijo}`],
  );
...
async function asegurarCaja(idEmp, idSuc) {
  const [abiertas] = await db.query(
    `SELECT idSesionCaja FROM sesion_caja
    WHERE idEmp=? AND estado='ABIERTA' ORDER BY idSesionCaja DESC LIMIT 1`,
    [idEmp],
  );
...
async function ejecutar() {
  ...
  comprobar(
    'Pedido reserva 10 a 8',
    aprobado.status === 201 && (await stock()) === 8,
    `HTTP ${aprobado.status}, stock ${await stock()}`,
  );
  const [snapshot] = await db.query(
    `SELECT bancoSnapshot,titularSnapshot,clabeSnapshot,
    numeroCuentaSnapshot,instruccionesSnapshot FROM pedido_cliente WHERE idPedido=?`,
    [aprobado.body.idPedido],
  );
  comprobar(
    'Pedido guarda cinco snapshots',
    snapshot[0].bancoSnapshot &&
      snapshot[0].titularSnapshot &&
      snapshot[0].clabeSnapshot &&
      snapshot[0].numeroCuentaSnapshot &&
      snapshot[0].instruccionesSnapshot,
  );
  r = await subirComprobante(aprobado.body.idPedido, clienteToken);
  comprobar(
    'Comprobante conserva stock 8',
    r.status === 200 && r.body.estado === 'EN_REVISION' && (await stock()) === 8,
  );
  r = await peticion('/admin/pedidos', {}, adminToken);
  comprobar(
    'Admin lista sólo pedidos de su sucursal',
    r.status === 200 && r.body.some((p) => p.idPedido === aprobado.body.idPedido),
  );
  r = await peticion(`/admin/pedidos/${aprobado.body.idPedido}`, {}, adminToken);
  comprobar(
    'Admin obtiene detalle seguro',
    r.status === 200 && !('comprobanteRuta' in r.body) && r.body.cliente.correo,
  );
  ...
  comprobar(
    'Aprobación crea venta y conserva stock 8',
    r.status === 200 && r.body.estado === 'PAGADO' && r.body.idVenta && (await stock()) === 8,
  );
  ...
  const [detallePedido] = await db.query('SELECT * FROM detalle_pedido_cliente WHERE idPedido=?', [
    aprobado.body.idPedido,
  ]);
  comprobar(
    'Venta online usa transferencia sin caja',
    ventas[0].metodoPago === 'TRANSFERENCIA' && ventas[0].idSesionCaja === null && ventas[0].montoRecibido === null,
  );
  comprobar('Venta total coincide con pedido', Number(ventas[0].total) === Number(aprobado.body.total));
  comprobar(
    'Detventa conserva precio y subtotal históricos',
    detallesVenta.length === detallePedido.length &&
  ...
  comprobar(
    'Historial identifica la venta online',
    r.status === 200 && r.body.some((v) => v.idVenta === idVentaAprobada && v.origenVenta === 'ONLINE'),
  );
  r = await peticion(`/ventas/${idVentaAprobada}`, {}, adminToken);
  comprobar(
    'Detalle de venta online admite caja nula',
    r.status === 200 &&
      r.body.origenVenta === 'ONLINE' &&
      r.body.idSesionCaja === null &&
      r.body.folioPedido === aprobado.body.folio,
  );
  r = await peticion(`/admin/pedidos/${aprobado.body.idPedido}/aprobar`, { method: 'POST', body: '{}' }, adminToken);
  const [cuentaVentas] = await db.query('SELECT COUNT(*) total FROM venta WHERE idVenta=?', [idVentaAprobada]);
  comprobar(
    'Doble aprobación no duplica venta',
    r.status === 409 && Number(cuentaVentas[0].total) === 1 && (await stock()) === 8,
  );
  ...
  comprobar(
    'PAGADO pasa a LISTO sin tocar stock',
    r.status === 200 && r.body.estado === 'LISTO' && (await stock()) === 8,
  );
  r = await peticion(`/admin/pedidos/${aprobado.body.idPedido}/entregar`, { method: 'POST', body: '{}' }, adminToken);
  comprobar(
    'LISTO pasa a ENTREGADO sin tocar stock',
    r.status === 200 && r.body.estado === 'ENTREGADO' && (await stock()) === 8,
  );
  r = await peticion(
    `/ventas/${idVentaAprobada}/cancelar`,
    { method: 'POST', body: JSON.stringify({ motivo: 'No debe cancelarse' }) },
    adminToken,
  );
  comprobar('Venta online no se cancela por endpoint POS', r.status === 409 && (await stock()) === 8);

  await db.query('UPDATE productos SET existenciaPro=10,precioVentaPro=? WHERE idPro=?', [
    precioOriginal,
    producto.idPro,
  ]);
  ...
  r = await peticion(
    `/admin/pedidos/${rechazado.body.idPedido}/rechazar`,
    {
      method: 'POST',
      body: JSON.stringify({ motivo: 'El comprobante no corresponde al monto.' }),
    },
    adminToken,
  );
  ...
  comprobar(
    'Rechazo conserva comprobante y motivo',
    Boolean(comprobanteRechazado) && Boolean(r.body.comprobante) && r.body.motivoRechazo,
  );
  ...
  const [aprobar, rechazar] = await Promise.all([
    ...
    peticion(
      `/admin/pedidos/${carrera.body.idPedido}/rechazar`,
      { method: 'POST', body: JSON.stringify({ motivo: 'Carrera controlada' }) },
      adminToken,
    ),
  ]);
  ...
  comprobar(
    'Carrera aprobar/rechazar tiene un ganador',
    exitosos.length === 1 && conflictos.length === 1 && [8, 10].includes(stockCarrera),
    `aprobar ${aprobar.status}, rechazar ${rechazar.status}, stock ${stockCarrera}`,
  );

  const sufijoSucursal = crypto.randomUUID().slice(0, 8);
  const [sucursal] = await db.query('INSERT INTO sucursal(nombreSuc) VALUES (?)', [
    `Sucursal prueba F3 ${sufijoSucursal}`,
  ]);
  idSucursalTemporal = Number(sucursal.insertId);
  const [pedidoAjeno] = await db.query(
  ...
  r = await peticion('/admin/pedidos', {}, adminToken);
  comprobar(
    'Listado excluye pedidos de otra sucursal',
    r.status === 200 && !r.body.some((p) => p.idPedido === pedidoAjeno.insertId),
  );
  ...
  r = await peticion(
    '/ventas',
    {
      ...
      body: JSON.stringify({
        ...
        items: [{ idPro: producto.idPro, cantidad: 1 }],
        metodoPago: 'EFECTIVO',
  ...
  ventasPrueba.add(Number(r.body.idVenta));
  r = await peticion(
    `/ventas/${r.body.idVenta}/cancelar`,
    {
      method: 'POST',
      body: JSON.stringify({ motivo: 'Cancelación POS de prueba' }),
    },
    adminToken,
  );
  ...
  console.log(
    JSON.stringify(
      {
        resultado: 'FASE_3_BACKEND_OK',
        pruebas: resultados,
        inventarioAprobacion: '10 -> 8 -> 8 -> 8 -> 8',
        inventarioRechazo: '10 -> 8 -> 10',
      },
      null,
      2,
    ),
...
async function limpiar() {
  ...
  try {
    ...
    for (const idPedido of pedidosPrueba) {
      const [rows] = await db.query('SELECT idVenta,comprobanteRuta FROM pedido_cliente WHERE idPedido=? FOR UPDATE', [
        idPedido,
      ]);
    ...
    if (producto)
      await db.query('UPDATE productos SET existenciaPro=?,precioVentaPro=? WHERE idPro=?', [
        stockOriginal,
        precioOriginal,
        producto.idPro,
      ]);
...
ejecutar()
  .catch((error) => {
    console.error('FASE_3_BACKEND_FAIL', error.message);
    process.exitCode = 1;
  })
  .finally(limpiar);
