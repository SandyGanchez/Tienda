...
export class CarritoService {
  ...
  readonly totalMostrado$ = this.items$.pipe(map(items => items.reduce((total, item) => total + item.precioMostrado * item.cantidad, 0)));

  get items(): ItemCarrito[] { return this.subject.value; }
  get cantidadTotal(): number { return this.items.reduce((total, item) => total + item.cantidad, 0); }
  get totalMostrado(): number { return this.items.reduce((total, item) => total + item.precioMostrado * item.cantidad, 0); }
  ...
  agregar(producto: ProductoParaCarrito): boolean {
    ...
    const actualizados = existente
      ? this.items.map(item => item.idPro === existente.idPro
        ? { ...item, cantidad: item.cantidad + 1, precioMostrado: precio, stockConocido: stock, imagen: producto.imagenPro, presentacion }
        : item)
      : [...this.items, {
          idPro: Number(producto.idPro), nombre: producto.nombrePro, precioMostrado: precio,
          cantidad: 1, imagen: producto.imagenPro, stockConocido: stock, presentacion
        }];
  ...
  incrementar(idPro: number): boolean {
    ...
    this.actualizar(this.items.map(actual => actual.idPro === idPro ? { ...actual, cantidad: actual.cantidad + 1 } : actual));
    return true;
  }
  ...
  eliminar(idPro: number): void { this.actualizar(this.items.filter(item => item.idPro !== idPro)); }
  vaciar(): void { this.actualizar([]); }

  actualizarDisponibilidad(productos: Array<{ idPro: number; existenciaPro: number | null; precioVentaPro: number }>): void {
    ...
    const actualizados = this.items.reduce<ItemCarrito[]>((resultado, item) => {
      ...
      const precio = Number(producto.precioVentaPro);
      resultado.push({ ...item, stockConocido: stock, cantidad: Math.min(item.cantidad, stock),
  ...
  private leer(): ItemCarrito[] {
    try {
      ...
      return guardado.filter(this.esItemValido).map(item => ({ ...item, cantidad: Math.min(item.cantidad, item.stockConocido) }));
    } catch { return []; }
  }
  ...
  private esItemValido(valor: unknown): valor is ItemCarrito {
    ...
    return typeof item.idPro === 'number' && Number.isInteger(item.idPro) && typeof item.nombre === 'string' &&
      typeof item.precioMostrado === 'number' && Number.isFinite(item.precioMostrado) && item.precioMostrado >= 0 &&
      typeof item.cantidad === 'number' && Number.isInteger(item.cantidad) && item.cantidad > 0 &&
      typeof item.stockConocido === 'number' && Number.isInteger(item.stockConocido) && item.stockConocido > 0 && item.cantidad <= item.stockConocido;
  }
}
