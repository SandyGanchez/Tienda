...
export class SyncService {
  ...
  private async iniciar(): Promise<void> {
    ...
    await Network.addListener('networkStatusChange', (s) => {
      ...
      if (s.connected) void this.sincronizarPendientes();
    });
  ...
  async sincronizarPendientes(): Promise<void> {
    ...
    try {
      const cola = await this.sqlite.pendientesSync();
      for (const op of cola.filter((x) => x.estado === 'PENDIENTE')) {
        try {
          const p = JSON.parse(op.payload);
          if (op.tipo === 'APERTURA') await firstValueFrom(this.cajas.abrir(p.uuidSesionCaja, p.fondoInicial));
